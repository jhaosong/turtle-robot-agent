...
def main():
    ...
    os.environ.setdefault("ROSA_TRACE", "true")
    parser = argparse.ArgumentParser()
    parser.add_argument("--streaming", action="store_true")
    parser.add_argument("legacy_args", nargs="*")
    args = parser.parse_args()

    streaming = args.streaming or any(
        arg.strip().lower() == "streaming:=true" for arg in args.legacy_args
    )
    turtle_tools.init_ros_turtle_tools()
    ...
    try:
        ...
        turtle_tools.shutdown_ros_turtle_tools()
        if rclpy.ok():
            rclpy.shutdown()
