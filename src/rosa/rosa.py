...
class ROSA:
    ...
    def __init__(
        ...
        self.__trace_enabled = os.getenv("ROSA_TRACE", "false").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
            ...
            async for event in self.__executor.astream_events(
                ...
                # Handle chain end events
                    ...
                    if chain_output:
                        ...
                        self._trace("CHAIN END", {"output": chain_output})
                        if not final_emitted:
                            final_emitted = True
                            yield {"type": "final", "content": chain_output}

            if final_output and not final_emitted:
    ...
    def _trace(self, label: str, payload: Any):
        if not self.__trace_enabled:
            return
        print(f"\n[ROSA TRACE] {label}")
        if isinstance(payload, str):
            print(payload)
        else:
            print(pprint.pformat(payload, sort_dicts=False))

    def _trace_query(self, query: str):
        if not self.__trace_enabled:
            return
        self._trace("USER QUERY", query)
        try:
            messages = self.__prompts.format_messages(
                input=query,
            ...
            self._trace("FORMATTED PROMPT", rendered)
        except Exception as exc:
            self._trace("FORMATTED PROMPT ERROR", str(exc))

    def _trace_executor_result(self, result: Dict[str, Any]):
        if not self.__trace_enabled:
            return
        self._trace("EXECUTOR RESULT", result)
