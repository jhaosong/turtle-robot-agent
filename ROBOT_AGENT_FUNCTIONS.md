...
| `circle_object_for_inspection` | `label: str`, `color: str | None`, `viewpoint_count: int = 4`, `radius_m: float | None` | In one call, triangulate a visible object, plan a fixed evenly spaced orbit, then navigate, align once, and photograph every viewpoint. |
...
| `wait_seconds` | `seconds: float` | Wait for a bounded duration. |
...
Planner output contains `objective`, up to ten `steps`,
...
| `CircleObjectForInspectionInput` | `label`, optional `color`, `viewpoint_count=4`, optional `radius_m` | Deterministic object localization and complete multi-view capture request. |
...
| `_get_detector` | None | Lazily load YOLOE only when a perception tool needs it. |
...
| `_inspection_failure` | `extra`, `error`, optional `retryable`, `status` | Build the normalized failure result shared by all multi-view inspection failure paths. |
| `_resolve_inspection_viewpoint` | `target`, `object_pose` | Evaluate one fixed orbit pose and, only if blocked, rank a small candidate fan generated by the existing baseline helper. |
| `_circle_object_for_inspection` | `label`, optional `color`, `viewpoint_count`, optional `radius_m` | Measure two bearings without pre-centering, triangulate directly, generate the inspection viewpoints once, and internally execute all captures. |
| `_wait_for` | `seconds: float` | Validate and execute a bounded wait. |
| `build` | None | Return the model-visible `StructuredTool` objects listed at the top of this document. |
...
| `_hold_zero_velocity` | duration | Hold zero velocity for a bounded settling window so residual controller output drains before visual control. |
...
| `update_navigation_status` | `status` | Store the latest verified navigation status. |
